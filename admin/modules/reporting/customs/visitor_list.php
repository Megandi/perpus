...
<?php
    ...
    if (isset($_GET['id_name']) AND !empty($_GET['id_name'])) {
        ...
        $criteria .= ' AND (vc.member_id LIKE \'%'.$id_name.'%\' OR m.member_name LIKE \'%'.$id_name.'%\')';
    }
    // register date
    ...
    echo '</script>';
	   $xlsquery = 'SELECT IF(vc.member_id IS NOT NULL, vc.member_id, \'NON-MEMBER\')  AS \''.__('Member ID').'\'',
         ...
         'IF(mt.member_type_name IS NOT NULL, mt.member_type_name, \'NON-MEMBER\') AS \''.__('Membership Type').'\'',
         'vc.activities AS \''.__('Visitor Activities').'\'',
         'vc.checkin_date AS \''.__('Visit Date').'\' FROM '.$table_spec.' WHERE '.$criteria. ' ORDER BY vc.member_id ASC';
